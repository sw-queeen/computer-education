...
(function () {
  ...
  const C = (typeof SITE_CONFIG !== 'undefined') ? SITE_CONFIG : {
    summary: { intro:'개요', learning:'학습자의 학습 과정', teacherRole:'교사의 역할 & 수업 방법', essay:'논술 답안 구조', check:'체크 포인트' },
  ...
  function renderSummary() {
    ...
    const steps = [
      ['주의집중', '1. 주의집중 획득★', '수업이 이루어지기 위해 학습자의 주의력을 획득해야 한다.'],
      ['기대', '2. 학습목표 제시', '수업 후 얻게 되는 수업결과를 명확히 알려줌으로써 기대감과 동기를 향상시킨다.'],
      ['장기→단기 인출', '3. 선수학습 회상', '새로운 지식은 선행학습을 기반으로 이루어지므로, 이전 학습내용을 먼저 확인한다.'],
      ['선택적 지각', '4. 자극자료 제시★', '학습자에게 학습 내용을 제시하는 단계. 독특한 특징을 부각하면 기억하기 쉽다. 선택적 지각 촉진.'],
      ['의미의 부호화', '5. 학습안내 제공★', '학습 내용들을 종합하고 상관관계를 제시하여 장기기억을 돕는다. 부호화·정교화·스캐폴딩 활용.'],
      ['반응', '6. 수행 유도★', '학습한 내용을 유사한 실제 문제 상황에 적용해보는 단계.'],
      ['강화', '7. 피드백 제공', '성공 수행에는 즉각 강화(칭찬), 실패 시에는 재학습 기회 제공 후 긍정적 피드백.'],
      ['인출과 강화', '8. 형성평가 (수행평가)', '수업목표 달성 여부를 평가. 단순암기가 아닌 수행평가로 이해 점검.'],
      ['일반화', '9. 파지와 전이 증진', '연습 기회를 계속 제공하여 장기기억 저장(파지)과 다른 상황에의 적용(전이)을 증진한다.'],
    ];
    ...
    <div class="detail-section">
      <div class="detail-section-title" style="--subject-accent:${ACCENT};">학습의 조건 (x2)</div>
      <div style="display:flex;flex-direction:column;gap:8px;font-family:${FONT};">
        ${[
          ['외적 조건', '학습자 외부에서 가해지는 다양한 <strong>교수사태</strong> — 9가지 수업사태가 이에 해당'],
          ['내적 조건', '학습자의 내적 인지과정 — 주의집중·기대·부호화·파지·전이 등'],
        ].map(([t,d],i) => `
          <div style="display:flex;gap:10px;align-items:flex-start;padding:10px 14px;background:var(--bg-surface,#F0EDE8);border-radius:8px;font-size:13px;line-height:1.8;">
    ...
    <div class="detail-section">
      <div class="detail-section-title" style="--subject-accent:${ACCENT};">9가지 수업사태 (내적 과정 → 외적 사태)</div>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px;font-family:${FONT};">
          <thead>
            <tr style="background:${BG};">
              ...
              <th style="padding:7px 10px;text-align:left;font-weight:700;color:${ACCENT};border-bottom:2px solid ${ACCENT};">수업사태 (외적)</th>
              <th style="padding:7px 10px;text-align:left;font-weight:700;color:${ACCENT};border-bottom:2px solid ${ACCENT};">설명</th>
            </tr>
          </thead>
          <tbody>
            ${steps.map(([inner,outer,desc],i) => `
              <tr style="border-bottom:1px solid var(--border-light,rgba(0,0,0,.07));${i%2?'background:var(--bg-page,#faf9f7)':''}">
                <td style="padding:7px 10px;color:${ACCENT};font-weight:600;white-space:nowrap;">${inner}</td>
                <td style="padding:7px 10px;font-weight:700;color:var(--text-primary,#2C2825);white-space:nowrap;">${outer}</td>
    ...
    <div class="detail-section">
      <div class="detail-section-title" style="--subject-accent:${ACCENT};">5가지 수업목표 (학습결과)</div>
    ...
    <div class="detail-section">
      ...
      <div style="display:flex;flex-direction:column;gap:6px;font-family:${FONT};font-size:13px;">
        ${[
          { label:'기본 개념', items:['학습목표에 따라 상이한 교수방법 처방', '교수활동은 인간의 내적 학습과정에 맞추어 설계해야 함'] },
          { label:'9가지 수업사태 핵심 3가지', items:['★ 자극자료 제시 — 선택적 지각 촉진', '★ 학습안내 제공 — 부호화·정교화·스캐폴딩', '★ 수행 유도 — 실제 문제 상황 적용'] },
          { label:'5가지 수업목표', items:['언어정보(군집분석), 지적기능(위계분석)', '인지전략, 태도(통합분석), 운동기능(절차분석)'] },
        ].map(({label, items}) => `
    ...
    <div class="detail-section">
      ...
      <div style="display:flex;flex-direction:column;gap:8px;font-family:${FONT};font-size:12px;">
        ${[
          ...
          ['핵심 암기', '#2D8A6A', '★ 4단계 선택적 지각, ★ 5단계 의미의 부호화, ★ 6단계 반응 — 3개가 빈출 짝꿍'],
          ['핵심 암기', '#2D8A6A', '수업사태 순서 암기: 주의→목표→선수→자극→안내→수행→피드백→평가→파지'],
  ...
  function renderExam() {
    ...
    <div class="detail-section">
      ...
      <div style="background:${BG};border-radius:10px;padding:13px 16px;font-size:12px;line-height:2;color:var(--text-primary,#2C2825);font-family:${FONT};">
        ...
        <span style="font-size:11px;color:var(--text-tertiary,#A09890);">★★★★ S급 빈출. 9가지 수업사태 + 5가지 수업목표 세트로 완벽 암기 필수.</span>
      </div>
    </div>
    <div class="detail-section">
