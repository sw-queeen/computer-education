...
(function () {
  ...
  const ACCENT = '#00e5c7';
  const BG     = '#002a24';
  const MID    = 'var(--border-mid,#2f7a2a)';
  ...
  if (!document.getElementById('marcia-viz-style')) {
    ...
    s.textContent = `
      #marcia-wrap { font-family:${FONT}; max-width:100%; color:var(--text-primary,#33ff00); }
      ...
      #marcia-wrap .mv-tab.on { background:${BG}; color:#4dffe3; border-color:${ACCENT}; }
      ...
      #marcia-wrap .mv-mid-label {
        ...
        color:var(--text-secondary,#5be03f);
        padding:4px 10px; border-radius:0;
        background:var(--bg-surface,#131813);
        white-space:nowrap;
      }
      #marcia-wrap .mv-cell {
      ...
      #marcia-wrap .mv-cell:hover { transform:translateY(-2px); }
      ...
      #marcia-wrap .mv-cell-desc { font-size:11px; line-height:1.65; color:var(--text-secondary,#5be03f); }
      #marcia-wrap .mv-cell-keyword {
        margin-top:7px; font-size:11px; font-weight:700;
        color:var(--cell-accent); padding:5px 9px;
        background:rgba(0,0,0,.35); border-radius:0; display:none;
      }
      #marcia-wrap .mv-cell.on .mv-cell-keyword { display:block; }
      #marcia-wrap .mv-cell-example {
        margin-top:5px; font-size:11px; color:var(--text-tertiary,#34a02c); display:none; line-height:1.6;
      }
      ...
      #marcia-wrap .mv-compare-table th {
        ...
        letter-spacing:.04em; border-bottom:2px solid var(--border-light,#1f521f);
        background:var(--bg-surface,#131813);
      }
      #marcia-wrap .mv-compare-table td {
        padding:9px 12px; border-bottom:1px solid var(--border-light,#1f521f);
        line-height:1.6; vertical-align:top;
      }
      #marcia-wrap .mv-compare-table tr:hover td { background:var(--bg-surface,#131813); }

      /* ── 발달 경로 ── */
      #marcia-wrap .mv-path-item {
        display:flex; align-items:flex-start; gap:12px;
        padding:12px 0; border-bottom:1px solid var(--border-light,#1f521f);
      }
  ...
  const STATUSES = [
    {
      ...
      bg: '#2a1a00', accent: '#ffb000', text: '#ffc94d',
    ...
    },
    {
      ...
      bg: '#002a24', accent: '#00e5c7', text: '#4dffe3',
    ...
    },
    {
      ...
      bg: '#1a002a', accent: '#b06bff', text: '#c99bff',
    ...
    },
    {
      ...
      bg: '#001a2a', accent: '#33aaff', text: '#6cc4ff',
    ...
    return `
      <div class="mv-matrix-wrap">
        ...
        <div style="display:flex;align-items:center;justify-content:center;">
          <div style="width:6px;height:6px;border-radius:0;background:var(--border-mid,#2f7a2a);"></div>
        </div>
        <div class="mv-mid-label">위기 ○</div>
      ...
      <div style="text-align:center;padding:8px 0 2px;font-size:11px;color:var(--text-tertiary,#34a02c);font-family:${FONT};">
  ...
  function renderCompare() {
    ...
    const accents = ['#ffb000', '#00e5c7', '#b06bff', '#33aaff'];
    const rows = [
      ['위기(탐색)', '✕', '○', '✕', '○'],
      ['전념(몰입)', '○', '○', '✕', '✕'],
        ...
        <table class="mv-compare-table" style="min-width:460px;">
          ...
          <tbody>
            ${rows.map(([label,...cells]) => `
              <tr>
                <td style="font-weight:700;color:var(--text-secondary,#5be03f);font-size:11px;">${label}</td>
                ${cells.map((v,i) => `<td style="color:${v==='○'?accents[i]:v==='✕'?'var(--text-tertiary,#34a02c)':'var(--text-primary,#33ff00)'};${v==='○'?'font-weight:700;':''}">${v}</td>`).join('')}
              </tr>`).join('')}
          </tbody>
        </table>
      </div>
      <div style="margin-top:14px;background:${BG};border-radius:0;padding:12px 16px;font-size:12px;line-height:1.8;color:var(--text-primary,#33ff00);font-family:${FONT};">
        <strong style="color:${ACCENT};">핵심 암기 공식</strong><br>
        위기 ✕ + 전념 ○ = <strong style="color:#ffb000;">유실</strong> &nbsp;|&nbsp;
        위기 ○ + 전념 ○ = <strong style="color:#00e5c7;">성취</strong> &nbsp;|&nbsp;
        위기 ✕ + 전념 ✕ = <strong style="color:#b06bff;">혼미</strong> &nbsp;|&nbsp;
        위기 ○ + 전념 ✕ = <strong style="color:#33aaff;">유예</strong>
      </div>`;
  }

  function renderPath() {
    const paths = [
      { from:'정체감 혼미', to:'정체감 유예', desc:'아무런 탐색도 하지 않다가 위기를 경험하고 적극적으로 탐색을 시작한다.', color:'#33aaff', bg:'#33aaff' },
      { from:'정체감 유예', to:'정체감 성취', desc:'탐색 과정을 거쳐 자신만의 가치관·목표를 확립하고 전념하게 된다. 가장 건강한 발달 경로.', color:'#00e5c7', bg:'#00e5c7' },
      { from:'정체감 유실', to:'정체감 성취', desc:'남의 가치관을 따르다가 스스로 탐색하고 재정립하는 과정을 거쳐 성취에 도달할 수 있다.', color:'#00e5c7', bg:'#00e5c7' },
      { from:'정체감 유실', to:'정체감 혼미', desc:'외부 기대가 무너지면 탐색 의지 없이 방황 상태(혼미)로 빠질 수 있다.', color:'#b06bff', bg:'#b06bff' },
    ];

    return `
      <div style="font-size:12px;color:var(--text-secondary,#5be03f);margin-bottom:14px;line-height:1.8;font-family:${FONT};">
      ...
      ${paths.map((p,i) => `
        <div class="mv-path-item">
          ...
          <div style="flex:1;font-family:${FONT};">
            ...
            <div style="font-size:12px;color:var(--text-secondary,#5be03f);line-height:1.65;">${p.desc}</div>
          </div>
        </div>`).join('')}`;
  }
