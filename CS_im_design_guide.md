...
> **⚠ 2026 디자인 시스템 전환 — Glassmorphism (Liquid Glass, Cool Violet)**
> 사이트 전체가 다크 전용 **Terminal CLI** 테마 → **Serif** 테마 → **Neumorphism** 테마를 거쳐, 현재는 **Glassmorphism** 테마로 전환됨(`css/variables.css` 전면 재작성). 아래 §11(디자인 토큰)·§15(6색 파스텔 팔레트)는 최초 라이트 테마 당시 기록이며 세부 수치는 다를 수 있으나, **"라이트 배경 + 6색 파스텔 과목 팔레트"라는 구조 자체는 Glassmorphism에서도 그대로 유지**됩니다. 실제 라이브 토큰 값은 항상 `css/variables.css`가 유일한 소스입니다.
> **Glassmorphism 디자인 시스템의 전체 원문 스펙(철학·토큰·컴포넌트 규칙)은 [`DESIGN_SYSTEM.md`](./DESIGN_SYSTEM.md)에 별도로 정리되어 있으며, 시각 디자인을 다루는 모든 작업은 그 문서를 일관되게 따릅니다.** 핵심 사항:
> - 배경은 라이트 전용(다크모드 없음), 고정(fixed) 라벤더→페리윙클→민트 그라디언트(`--bg-gradient`) — 카드·탑바·사이드바·인풋은 이 그라디언트 위에 뜨는 **반투명(rgba) + `backdrop-filter: blur()` 유리 표면**이며, 사진·오브 배경은 사용하지 않음(차분한 그라디언트만)
> - 폰트: 표제(`--font-heading`)는 `Plus Jakarta Sans`, 본문(`--font-body`)은 `DM Sans` — 둘 다 한글 글리프가 없어 한글 텍스트는 시스템 산세리프로 자동 폴백됨(의도된 동작, 폰트 자체는 이번 전환에서 변경 없음)
> - **사이트 공통 액센트(버튼·링크·포커스링)는 소프트 바이올렛**(`--primary`/`--accent: #6C63FF`, 라이트 톤 `--secondary`/`--accent-secondary: #8B84FF`) — 뉴모피즘 때와 동일한 값 유지
> - **실제 보더는 여전히 사용하지 않음** — 카드/패널의 "테두리"는 `border` 속성이 아니라 `box-shadow`의 인셋 상단 하이라이트(sheen)로 표현. `--border-light`/`--border-mid`는 표·탭 밑줄 등 유리 패널 **내부** 콘텐츠 구분선 전용으로, 반투명 다크 헤어라인(`rgba(43,52,64,α)`)으로 재정의됨
> - `--radius-*`는 뉴모피즘 때와 동일(12~32px, 버튼 16px·카드 32px) — 레이아웃은 그대로, 재질만 "폼→유리"로 교체. `box-shadow`는 바깥 드롭섀도+안쪽 상단 하이라이트+안쪽 하단 베벨의 3중 유리 그림자(`--shadow-extruded/-hover/-sm`, `--shadow-inset/-deep/-sm` — 토큰 이름은 그대로, 값만 유리 물리로 재정의)로 전면 교체, 모든 카드/버튼/웰에 `backdrop-filter: var(--glass-blur)` 추가
> - 버튼·칩·퀴즈 옵션·탭 등 **클릭 가능한 요소는 탭 시 ~900ms 짜리 "젤리 워블"** (`csWobble` 키프레임, `js/ui.js`의 `CS.wobble` 위임 클릭 핸들러가 자동 부착) 튕김 애니메이션이 함께 재생됨. `prefers-reduced-motion`에서는 자동 비활성화
> - 6색 파스텔 팔레트(`--c-pink/coral/yellow/mint/blue/lilac-*`)는 이번 전환에서 **`-bg` 값만 반투명 유리톤(rgba, α≈0.55)으로 변환**됨. `-mid`/`-accent`/`-text`는 §15 스프레드시트 원본 HEX 값 그대로(불투명) 유지 — 텍스트·아이콘 가독성 담당. 과목 구분은 이 팔레트가 전담하고, 사이트 전역 바이올렛 액센트·그라디언트 베이스와는 별개 레이어로 유지됨. 추가로 `--c-chrome-silver-*`·`--c-ink-black-*` 2종 중립 토큰 존재(과거 액센트 원재료, 참고용, 변경 없음)
> - 탭 구조·콘텐츠 작성 규칙(§2~§10, §17~§19)은 변경 없음. 새 개념을 추가할 때 색상 값을 하드코딩하지 말고 항상 `var(--...)` 토큰을 참조할 것 (§13 참고).
> - `test-design.html`은 이번 전환 범위에서도 제외(원래도 본 사이트와 무관한 실험 페이지, §16 참고). 단, `css/components.css`의 공용 토큰을 그대로 불러 쓰므로 배경·그림자 톤은 함께 바뀜(의도된 부작용).
> - 카드·버튼 hover는 그림자가 한 단계 더 깊어지는 `-hover` 톤(더 밝은 sheen) + 미세한 리프트(-1~2px)를 함께 사용, active(press) 시에는 인셋 그림자로 눌리는 느낌 + 젤리 워블을 함께 표현 — "모든 상호작용은 촉각적 피드백" 원칙은 Glassmorphism에서도 유지.
...
> 채도 높은 파스텔 톤에서 더 차분한 더스티·뮤트 톤으로 개정됨(출처: 색상 체계 스프레드시트). 색상 그룹과 교육학/교과교육학/전공 매핑 구조 자체는 변경 없음 — 색만 교체.

| 색상 | bg | text | mid(서브텍스트) | 교육학 | 교과교육학 | 전공 |
|---|---|---|---|---|---|---|
| 더스티 로즈 | `#EACFCF` | `#6B2F2F` | `#8F4A4A` | 교육과정 | 정보과 교육과정 | 인공지능 · 이산수학 |
| 테라코타 | `#EAD3BF` | `#6B3D1F` | `#8F5A34` | 교수학습 | 교수학습 방법 | 운영체제 · 프로그래밍언어론 |
| 뮤트 샌드 | `#EAE1CA` | `#5C4E2A` | `#7A6A3D` | 교육평가 | 평가론 | 자료구조 · 알고리즘 |
| 세이지 그린 | `#CFDCD2` | `#2E4A38` | `#44694F` | 교육심리 | 컴퓨팅 사고력(CT) | 데이터베이스 · 소프트웨어공학 |
| 더스티 블루 | `#CFDBE4` | `#24435A` | `#3D5F73` | 교육행정 | SW·AI 교육론 | 네트워크 · 정보보안 |
| 뮤트 라일락 | `#DBD2E2` | `#3F2E4A` | `#5A4468` | 생활지도·상담 | (미사용) | 컴퓨터구조 · 컴파일러 |
