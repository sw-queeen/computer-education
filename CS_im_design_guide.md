...
| 순서 | 탭 이름 | 내용 |
|---|---|---|
| 1 | 핵심 정리 | 개요 → 핵심 섹션들 → 논술 답안 구조 → 체크 포인트 |
| 2 | 둘러보기 | 인터랙티브 시각화 |
...
| 섹션 | 형식 | 예시 |
|---|---|---|
| 개요 | prim 배경색 박스 + 본문 텍스트 | 피아제·가네 개요 |
...
| 논술 답안 구조 | 무채색 카드 + 회색 원형 번호 | 공통 |
| 체크 포인트 | 왼쪽 컬러 border 카드 + 레이블 pill | 공통 |

### 원형 번호 색상 규칙
- **논술 답안 구조**: `#A09890` 회색 (중요도 강조 없음)
- **핵심 3단계 등 중요 항목**: `prim(ACCENT)` 색상
- **일반 나열**: `#A09890` 회색

### 강조 표현 규칙
- `★` 기호 사용 금지 → **색상** 또는 **볼드**로 대체
- 텍스트 내 중요 키워드: `<span style="color:ACCENT;font-weight:700;">`
- 테이블에서 중요 행: 텍스트 색상만으로 구분 (행 배경색 강조 금지 — 줄 구분용 배경색과 충돌)
- 컬럼명 전부 좌측 정렬
...
- 탭 전환: `.gv-tab.on` 스타일 (prim 배경 + 어두운 텍스트)
...
- 선지에 `(이것도 틀림)` 같은 메타 텍스트 금지
- 빈칸 문제: 질문의 빈칸 형식과 정답 형식이 일치해야 함  
  예: `( )의 배수` → 정답 `2`  /  `( )의 거듭제곱` → 정답 `2 (1, 2, 4, 8, …)`
...
연도 없는 항목은 자동으로 상단으로 정렬(`sortExamItems()`).
...
| 논술 답안 구조 번호에 prim 색상 | 회색(`#A09890`) 무채색 사용 |
